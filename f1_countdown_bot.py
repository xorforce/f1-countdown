...
class F1CountdownBot:
    def __init__(self, config_file: str = 'config.ini', debug_mode: bool = False):
        ...
        self.timezone = pytz.timezone(self.config.get('settings', 'timezone', fallback='Asia/Kolkata'))
    ...
    def _load_config(self, config_file: str, debug_mode: bool = False) -> configparser.ConfigParser:
        ...
        if not os.path.exists(config_file):
            logger.error(f"Config file {config_file} not found. Please create it based on config.ini.template")
            sys.exit(1)
        ...
        logger.info("Configuration loaded successfully (Twitter credentials from environment variables)")
        return config
    ...
    def _setup_twitter_api(self) -> tweepy.API:
        ...
        try:
            ...
            logger.info("Twitter API authenticated successfully using environment variables")
            return api
    ...
    def _find_next_and_last_races(self, current_year: int) -> Tuple[Optional[pd.Series], Optional[pd.Series], int]:
...
#F1 #Formula1 #Countdown"""
    ...
    def daily_tweet_generation(self):
        ...
        try:
            ...
            next_race, last_race, actual_year = self._find_next_and_last_races(current_year)
